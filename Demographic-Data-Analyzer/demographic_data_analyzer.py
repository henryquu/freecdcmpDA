...
def calculate_demographic_data(print_data=True):
    ...
    highest_earning_country_percentage = highest_earning_country / df.value_counts(subset='native-country') * 100
    highest_earning_country_percentage = highest_earning_country_percentage.sort_values(ascending=False)

    highest_earning_country = highest_earning_country_percentage.index[0]
    highest_earning_country_percentage = round(highest_earning_country_percentage[0], 1)
    
    # Identify the most popular occupation for those who earn >50K in India.
    top_IN_occupation = df[(df['salary'] == '>50K') & (df['native-country'] == 'India')].value_counts(subset='occupation').index[0]
